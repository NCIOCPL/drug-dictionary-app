...
module.exports = function (proxy, allowedHost) {
	return {
		...
		// Completely disable static file serving for Pa11y testing to prevent conflicts
		static: process.env.NODE_ENV === 'test' || process.env.CI ? false : {
			directory: paths.appPublic,
			publicPath: paths.publicUrlOrPath.slice(0, -1),
			watch: {
				ignored: ignoredFiles(paths.appSrc),
			},
			// Prevent conflicts with custom middleware
			serveIndex: false,
		},
		server: (() => {
