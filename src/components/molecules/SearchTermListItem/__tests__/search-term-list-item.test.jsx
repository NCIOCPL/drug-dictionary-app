...
import SearchTermListItem from '../search-term-list-item';

describe('<TermListItem />', () => {
	it('should display term, definition, data-cdr-id attribute with termId as value, and term link with prettyUrlName in href value', async () => {
		const itemIndex = 2;
		...
		render(
			<MemoryRouter initialEntries={['/']}>
				<SearchTermListItem itemIndex={itemIndex} prettyUrlName={prettyUrlName} term={term} termDefinition={termDefinition} termId={termId} termLinkPath={termLinkPath} />
			</MemoryRouter>
		);
		...
		const dfnElement = screen.getByText(term, { selector: 'dfn *' }).closest('dfn');
		expect(dfnElement).toHaveAttribute('data-cdr-id', `${termId}`);
	});

	it('should display term link with termId in href attribute value when there is no prettyUrlName', async () => {
		const itemIndex = 2;
		const term = 'c-Kit inhibitor PLX9486';
		const termDefinition = 'An orally bioavailable protein tyrosine kinase inhibitor of mutated forms of the tumor-associated antigen mast/stem cell factor receptor c-Kit (SCFR), with potential antineoplastic activity. Upon oral administration, c-Kit inhibitor PLX9486 binds to and inhibits specific c-Kit mutants. This may result in an inhibition of tumor cell proliferation in cancer cell types that overexpress these c-Kit mutations. c-Kit, a transmembrane protein and receptor tyrosine kinase, is overexpressed in solid tumors and hematological malignancies; it plays a key role in the regulation of cell differentiation and proliferation.';
		const termId = 770823;
		...
		render(
			<MemoryRouter initialEntries={['/']}>
				<SearchTermListItem itemIndex={itemIndex} term={term} termDefinition={termDefinition} termId={termId} termLinkPath={termLinkPath} />
			</MemoryRouter>
		);
		await waitFor(() => {
			const link = screen.getByRole('link', { name: term });
			expect(link).toHaveAttribute('href', '/def/770823');
		});
	});

	it('should display preferred name in place of definition when term definition is not present', async () => {
		const props = {
			itemIndex: 1,
	...
	it('should call trackingHandler function once when term link is clicked', async () => {
		const itemIndex = 2;
		const prettyUrlName = 'c-kit-inhibitor-plx9486';
		const term = 'c-Kit inhibitor PLX9486';
		const termDefinition = 'An orally bioavailable protein tyrosine kinase inhibitor of mutated forms of the tumor-associated antigen mast/stem cell factor receptor c-Kit (SCFR), with potential antineoplastic activity. Upon oral administration, c-Kit inhibitor PLX9486 binds to and inhibits specific c-Kit mutants. This may result in an inhibition of tumor cell proliferation in cancer cell types that overexpress these c-Kit mutations. c-Kit, a transmembrane protein and receptor tyrosine kinase, is overexpressed in solid tumors and hematological malignancies; it plays a key role in the regulation of cell differentiation and proliferation.';
		...
		render(
			<MemoryRouter initialEntries={['/']}>
				<SearchTermListItem itemIndex={itemIndex} prettyUrlName={prettyUrlName} term={term} termDefinition={termDefinition} termId={termId} termLinkTrackingHandler={termLinkTrackingHandler} termLinkPath={termLinkPath} />
			</MemoryRouter>
		);

		const termLink = screen.getByRole('link', { name: term });
		await waitFor(() => {
			expect(termLink).toHaveTextContent(term);
		});
